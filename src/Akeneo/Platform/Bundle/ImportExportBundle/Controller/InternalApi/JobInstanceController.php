...
class JobInstanceController
{
    ...
    protected function launchAction(Request $request, string $code): Response
    {
        ...
        if (null !== $file) {
            ...
            $remotePath = $code.'/'. $file->getClientOriginalName();
            ...
            $this->filesystem->writeStream($remotePath, $fileContent);
            if (is_resource($fileContent)) {
                fclose($fileContent);
            }

            $rawParameters = $jobInstance->getRawParameters();
            $jobFileLocation = new JobFileLocation($remotePath, true);
            $rawParameters['filePath'] = $jobFileLocation->encodeLocation();
